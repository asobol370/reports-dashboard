...
## 🚨 Формат CSV Google Ads UI (українська локаль) — типові пастки

**Структура файлу**:
```
Row 1:  «Звіт про кампанію»              ← ІГНОРУВАТИ (заголовок)
Row 2:  «3 серпня 2026 р. - 9 серпня…»   ← беремо тільки для формування label періоду
Row 3:  заголовки колонок українською
Row 4..N-M:  САМІ КАМПАНІЇ (одна кампанія = один рядок)
Row N-M..N:  агрегати «Усього: …» ← ⚠️ ВИКЛЮЧАТИ ці рядки повністю!
```

**Рядки-«пастки» (виключити з масиву кампаній)** — все, що починається з `Усього:`:
- `Усього: Кампанії` — сума всіх кампаній
- `Усього: Обліковий запис` — сума по акаунту (дубль)
- `Усього: Пошук` / `Усього: Розумна` / `Усього: Показ` / `Усього: Покупки` / `Усього: Створення попиту` / `Усього: Максимальна ефективність` — subtotals по типах

⚠️ **Якщо їх не виключити — spend/revenue подвоїться**. Це головна помилка LLM на цьому форматі.

**Обов'язкова валідація**: після парсингу `sum(spend по всіх твоїх кампаніях) == значення з рядка "Усього: Кампанії"`. Якщо не збігається — щось не так.

## Мапінг колонок CSV → поля JSON

| Колонка в CSV | Що це | Куди в data.json |
|---|---|---|
| `Кампанія` | назва | `campaigns[].name` |
| `Вартість` | витрати | `spend` / `cost` |
| `Конверсії` | покупки (primary+included) | `purchases` |
| `Цінність конв.` | виручка | `revenue` |
| `Вартість / конв.` | CPA | `cpa` |
| `Цінність / конв.` | ср. чек | `avg_check` |
| `Цінність конв./вартість` | ROAS (десятковий, множ на 100 для %) | `roas` |
| `Коефіц. конверсії` | CR (у %) | `cr_purchase` |
| `Покази` | impressions | `impressions` |
| `Кліки` | clicks | `clicks` |
| `Сер. ціна за клік` | CPC | `cpc` |
| `CTR` | CTR (у %) | `ctr` |
| `Частка показів у пошук. мережі` | IS | `impression_share` |
| `Частка пок. у верх. частині сторінки в пошуковій мережі` | IS Top | `top_is` |
| `Частка показів на найпершій позиції в пошуковій мережі` | IS Abs Top | `first_is` |
| `Усі конв.` | all conversions | для Add to Cart |
| `Цінність усіх конв.` | all conversions value | — |
| `Тип бюджету` / `Кампанія` name | тип кампанії | Search/PMax/Demand Gen/Display |

## Парсинг чисел (украинський формат)

```python
def parse_num(s):
    if s is None or s.strip() in ('', '--', '—', ' --', '-'):
        return None                     # немає даних
    s = s.strip()
    if s.startswith('<'):
        return None                     # «< 10%» — Google приховав точне значення
    s = s.replace(' ', '').replace(' ', '')   # nbsp + звичайні пробіли
    s = s.replace('грн.', '').replace('грн', '')   # валюта
    if s.endswith('%'):
        s = s[:-1]
    s = s.replace(',', '.')             # decimal
    return float(s)
...
Приклади:
- `"23381,33"` → `23381.33`
- `"386 524,12"` → `386524.12` (пробіл = nbsp тисячі)
- `"3,92%"` → `3.92`
- `"< 10%"` → `None`
- `" --"` → `None`
...
CSV з експорту «Звіт про кампанію» (без сегмента) — це **АГРЕГАТ за весь період**, там немає daily.
Для daily chart у дашборді потрібен окремий експорт з увімкненим **Segment → День** — тоді у CSV буде колонка «День» і по одному рядку на КОЖНИЙ день × кампанія.

Якщо ученик прислав тільки агрегований CSV без daily-розбивки — попередь його і згенеруй `chart.labels`/`chart.series` як 1 точку на весь період (не найкраще, але робоче).

## Приклади рядків, які ЗАВЖДИ треба ігнорувати

```
Усього: Кампанії, ...              ← агрегат (виключити)
Усього: Обліковий запис, ...       ← агрегат (виключити)
Усього: Пошук, ...                 ← subtotal (виключити)
Усього: Розумна, ...               ← subtotal (виключити)
Усього: Показ, ...                 ← subtotal (виключити)
Усього: Покупки, ...               ← subtotal (виключити)
Усього: Створення попиту, ...      ← subtotal (виключити)
Усього: Максимальна ефективність, ...  ← subtotal (виключити)
```

Пропускай ці рядки повністю. Не додавай як окрему кампанію. Використовуй лише для **валідації** (звірити свою суму).
